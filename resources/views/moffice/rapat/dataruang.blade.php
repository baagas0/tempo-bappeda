				...
				<input type="hidden" name="token" id="tokentemp" value="{{ Session::token() }}" />
					...
					<div class="row" style="background-color:white; padding:10px;">
						...
						<div class="col s4 row">
							...
							<div class="col s12">
								<div class="content-text">
									@foreach(json_decode($row->ruang) as $dd)
									<?php
										$ruang = \DB::table('m_ruang')->where('id', $dd)->first();
									?>
									<span class="{{ ($dd == 1 ? 'pink' : ($dd == 2 ? 'red' : ($dd == 3 ? 'blue' : ($dd == 4 ? 'purple' : ($dd == 5 ? 'indigo' : ($dd == 6 ? 'cyan' : 'green')))))) }}" style="color:white;padding:2px;margin:4px solid white;">{{ $ruang->name }}</span>
									@endforeach
								</div>
							</div> 
						</div>
