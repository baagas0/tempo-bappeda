						...
						<div class="col-xs-12 col-sm-6 col-lg-4">
							<article class="hb-post">
								<figure class="hb-postimage">
									...
									@break
									@endif
									<a href="{{ route('..pengumuman',$ig->id) }}"><img src="{{ asset($d) }}" style="height: 250px;width: auto;" alt="image description"></a>
									@endforeach
								</figure>
								<div class="hb-postcontent">
									<a href="{{ route('..pengumuman',$ig->id) }}" class="hb-postcategory">{{ ($ig->is_sosbud == 1) ? '#PPE' : '' }} {{ ($ig->is_bpsb == 1) ? '#Sosbud' : '' }} {{ ($ig->is_ekonomi == 1) ? '#Ekonomi' : '' }} {{ ($ig->is_infrastruktur == 1) ? '#Infrstruktur' : '' }} {{ ($ig->is_litbang == 1) ? '#Litbang' : '' }}</a>
									<div class="hb-posttitle">
										<h3><a href="{{ route('..pengumuman',$ig->id) }}">{{ Str::limit($ig->caption, 20) }}</a></h3>
									</div>
